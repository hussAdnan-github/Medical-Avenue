...
export default function HeroSection() {
  ...
  return (
    <section className="relative min-h-screen flex flex-col justify-center overflow-hidden">
      ...
      {/* ── Ripple rings (brand identity circles) ── */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[1] pointer-events-none hidden lg:block">
        <div className="relative w-0 h-0">
          {/* Ring 1 */}
          <div
            className="absolute rounded-full border border-brand-cream-300/10 animate-ripple"
            style={{ width: 500, height: 500, top: -250, left: -250 }}
          />
          {/* Ring 2 — delayed */}
          <div
            className="absolute rounded-full border border-brand-cream-300/8 animate-ripple-delay"
            style={{ width: 500, height: 500, top: -250, left: -250 }}
          />
          {/* Ring 3 — static subtle */}
          <div
            className="absolute rounded-full border border-white/5"
            style={{ width: 700, height: 700, top: -350, left: -350 }}
          />
        </div>
      </div>
      ...
      <div className="absolute top-1/2 right-24 -translate-y-1/2 z-[2] pointer-events-none hidden xl:block">
        <div className="relative w-0 h-0">
          {/* Orbit ring visual */}
          <div
            className="absolute rounded-full border border-brand-cream-300/10"
      ...
      <div className="absolute top-24 left-8 z-[3] pointer-events-none hidden lg:block">
        <div className="flex flex-col gap-1.5">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="bg-brand-cream-300/20 rounded-full"
              style={{ width: 32 - i * 6, height: 2 }}
            />
          ))}
        </div>
      </div>
      ...
      {/* ── Glow spot behind content ── */}
      <div
        className="absolute z-[1] pointer-events-none hidden lg:block"
        style={{
          right: "8%",
          top: "30%",
          width: 400,
        ...
        <div className="max-w-2xl">
          ...
          <ScrollReveal variant="up" delay={440}>
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              >
                ...
                <span className="relative z-10">{t("hero.btn_book")}</span>
              </Link>
              <Link
                to="/services"
          ...
          <ScrollReveal variant="up" delay={560}>
            <div className="flex flex-wrap gap-4 mt-12">
              {[t("hero.pillar_1"), t("hero.pillar_2"), t("hero.pillar_3")].map((p) => (
                <div
                  key={p}
