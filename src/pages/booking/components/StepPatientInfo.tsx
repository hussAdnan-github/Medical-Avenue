...
export default function StepPatientInfo({
  ...
  const doctor  = doctorsDetailed.find((d) => d.id === doctorId);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("https://readdy.ai/api/form/d7dei4ckpdh0a4ae003g", {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: data.toString(),
      });
      if (res.ok) {
        const bookingRef = `MC-${Date.now().toString(36).toUpperCase().slice(-6)}`;
        onSuccess({
          bookingRef,
