...
import { allPackages, packageCategories, type PackageCategory, type MedicalPackage } from "@/mocks/packagesData";
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {activeCategory === "all"
                ? `${allPackages.length} باقة موزعة على ${packageCategories.length} مستوى رعاية`
                : `${allPackages.filter((p) => p.category === activeCategory).length} باقة متاحة`}
            </p>
          </div>
          <Link
            to="/booking"
