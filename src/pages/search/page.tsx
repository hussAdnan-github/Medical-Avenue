...
import { doctorsDetailed } from "@/mocks/doctorsData";
...
export default function SearchPage() {
  ...
  const popularSearches = ["الطب الدقيق", "تحليل DNA", "Second Opinion", "القلب", "الجلدية", "الأطفال", "باقة شاملة", "VIP"];
        ...
        {!query && (
          <div className="mt-10 bg-white rounded-2xl border border-brand-cream-200 p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: "احجز موعداً",   path: "/booking",  icon: "ri-calendar-check-line", color: "bg-brand-forest-600 text-white hover:bg-brand-forest-700" },
                { label: "تصفح الباقات",  path: "/packages", icon: "ri-gift-line",            color: "bg-amber-50 text-amber-700 border border-amber-200 hover:bg-amber-100" },
                { label: "أطباؤنا",       path: "/doctors",  icon: "ri-user-heart-line",      color: "bg-brand-cream-100 text-brand-forest-700 border border-brand-cream-200 hover:bg-brand-cream-200" },
                { label: "تواصل معنا",    path: "/contact",  icon: "ri-phone-line",           color: "bg-gray-100 text-gray-700 hover:bg-gray-200" },
              ].map((item) => (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold whitespace-nowrap cursor-pointer transition-all hover:-translate-y-0.5 ${item.color}`}
                >
                  <div className="w-4 h-4 flex items-center justify-center">
          ...
          <div className="mt-6 bg-brand-forest-900 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 flex items-center justify-center bg-brand-cream-300/20 rounded-xl flex-shrink-0">
                <i className="ri-dna-line text-brand-cream-200 text-2xl"></i>
              </div>
              <div>
                <h4 className="font-black text-white text-sm">هل تبحث عن الطب الدقيق؟</h4>
                <p className="text-brand-cream-300 text-xs mt-0.5">
                  تحليل DNA · Second Opinion دولي · خطط علاجية مخصصة
                </p>
              </div>
            </div>
            <Link
